...
type PollUpdater struct {
	...
	// память, а не БД. Правило 5 («состояние дебаунса живёт в Postgres») этому
	// не противоречит: дебаунс решает судьбу карточки, а тут дебаунсится строка
	// лога. С правилом 4 («падать громко, а не пусто») натяжение есть, и оно
	// осознанное: тише становится сообщение о ШТАТНОМ состоянии общей базы, а
	// count в нём остаётся точным.
