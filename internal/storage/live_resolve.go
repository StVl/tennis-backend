...
//
// Кандидатом считается только отслеживаемый игрок, у которого совпадает
// нормализованное полное имя ЛИБО нормализованная фамилия. Ровно один кандидат
// — привязываем; ноль или несколько — отказ.
//
// Совпадение по одной фамилии здесь допустимо только потому, что выборка
// ограничена отслеживаемыми (около сотни человек) и требуется уникальность
// внутри неё. Среди всех 173 игроков базы фамилии повторяются, и там же лежат
// строки-заглушки вида "Boyer" или "Zheng M." — привязка к ним была бы
...
func MatchPlayerByName(ctx context.Context, pool *pgxpool.Pool, vendorName string) (int64, error) {
	...
	rows, err := pool.Query(ctx, `
		select p.id, p.display_name, coalesce(p.last_name, '')
		from players p
		where p.is_tracked`)
	if err != nil {
		return 0, err
