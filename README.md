...
| `APNS_KEY_BASE64` | — | содержимое `.p8` в base64. Так ключ передаётся на Railway: файл там положить некуда, а в base64 нет переводов строк, которые иначе теряются в переменной окружения. Приоритетнее пути |
| `APNS_KEY_PATH` | — | путь к `.p8` — для локальной работы. Нужен один из двух: путь или base64 |
