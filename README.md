...
- турнир источника должен резолвиться в наш розыгрыш через `external_ids` (`db/live_edition_ids.sql`). Незнакомый id — в `live_unmatched` с `reason='edition_unmapped'`; **это и есть очередь, по которой файл маппинга пополняется**, а не повод угадать по названию (у нас они спонсорские, у источника городские);
