...
- `import_key = livetennisapi:<id>` и `metadata.source` помечают строку нашей: по ним же работает уборка. Строки, оставшиеся `scheduled` спустя 48 часов после назначенного времени, переводятся в `cancelled` — иначе они навсегда становятся у обоих игроков «следующим матчем».
