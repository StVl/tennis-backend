...
- турнир источника должен резолвиться в наш розыгрыш через `external_ids` (`db/live_edition_ids.sql`), причём **только по подтверждённой привязке** (`confirmed_at is not null`). Половина сида выведена из каталога `/tournaments`: турнир он называет верно, но роль каждого id (основная сетка / квалификация / пара) не сообщает — а именно она решает, в какую сетку попадёт матч. Это единственное место, где догадка привела бы к записи строк в чужую таблицу, поэтому неподтверждённый id уходит в `live_unmatched` с `reason='edition_unmapped'`; **это и есть очередь, по которой файл маппинга пополняется**, а не повод угадать по названию (у нас они спонсорские, у источника городские);
