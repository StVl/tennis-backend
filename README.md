...
| Метод | Путь | Описание |
|---|---|---|
| `POST` | `/v1/dev/matches/{id}/live` | пометить матч живым: `200` + состояние флипа, `204` если уже живой |
...
```json
{
  "match_id": 482, "source": "dev", "external_key": null,
  "state": "on_court", "prior_status": "scheduled",
  "flipped_at": "2026-08-26T12:09:09Z", "last_seen_run_id": null,
  "status": "live", "scheduled_at": "2026-08-26T14:05:21Z"
}
```
...
- `prior_status` — то, что будет восстановлено; для завершённого матча это `completed`, и после `/finish` он именно туда и вернётся;
- каждый переход кладёт строку в outbox `live_events` — тот же механизм, который позже будет читать пушер;
- флип меняет `matches.status`, а значит матч сразу появляется в `GET /v1/matches?status=live` и включает `is_live` в виджете и на главной у обоих участников.
