...
| `LIVE_MATCHES_CAP` | `5` | потолок матчей в `/v1/users/me/live-matches` |
...
- выдача ограничена `LIVE_MATCHES_CAP` (по умолчанию 5) — столько же, сколько Live Activity клиент держит одновременно;
- только одиночки; парный разряд исключён;
- пока ни один матч не помечен живым, ответ — `{"items": []}`. Это норма, а не ошибка: поднять матч руками можно через `/v1/dev`.
