...
`gofmt -l .` currently reports drift in `internal/scheduler/scheduler.go` and `internal/storage/tournaments.go` (misaligned `var`/struct blocks). Format files you touch; don't reformat the rest as drive-by noise.
