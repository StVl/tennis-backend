...
--
-- Владелец этих таблиц — ЭТОТ сервис: он их определяет, применяет (на старте,
-- см. internal/storage/schema.go) и документирует. В tennis-data-storage они
-- не переносятся: тот репозиторий владеет схемой контента и о своих читателях
-- ничего знать не должен. Разделение по владельцу, а не по расположению:
-- в matches этот сервис меняет ровно один столбец, status, и всегда под
-- условием на текущее значение. См. docs/database.md.
--
-- Применить локально:
